...
class PositionalEncoding(nn.Module):
    def __init__(self, max_time_steps: int = 100, embedding_size: int = 512, n: int = 10000) -> None:
        super().__init__()
...
def main():
    ...
    betascheduler = BetaScheduler("cosine")

    betas = betascheduler(100)
    alphas = 1. - betas
    alpha_bar = torch.cumprod(alphas, dim=0)
    print(alpha_bar)

    # pos_encoding = PositionalEncoding()
    # # print(pos_encoding(torch.arange(1000)).shape)
    # ts = torch.randint(1, 100, (100,))
    # print(pos_encoding(ts)[0])
    # for i in range(1, 16):
    #     row = i // 4
    #     col = i % 4
