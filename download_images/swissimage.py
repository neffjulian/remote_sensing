...
def get_tiles(fpath_bbox: Path, out_dir: Path) -> None:
    """
    Query and download tiles SwissImage tiles from SwissTopo (10cm and 2m
    spatial resolution)

    :param fpath_bbox:
        geographic extent for which to download tiles
    :param out_dir:
    ...
    for resolution in scenes.keys():
        ...
        for url in url_list:
            ...
            bound_df = gpd.GeoDataFrame(geometry=[ds['B1'].bounds], crs=ds['B1'].geo_info.epsg)
            fname_bbox = fname.replace('.tif', '_bbox.geojson')
            bound_df.to_file(out_dir_res_year.joinpath(fname_bbox))

if __name__ == '__main__':
    data_dir = Path('../images')
    fpath_bbox = Path('../images/bounding_box/bounding_box_strickhof.geojson')
    out_dir = data_dir.joinpath('../images/swissimage')
